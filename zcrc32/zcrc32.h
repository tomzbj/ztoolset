...
#if !defined(ZCRC32_SOFT) && !defined(ZCRC32_LUT)
#define ZCRC32_SOFT
#endif
...
class ZCRC32 {
  public:
    ...
    static uint32_t calc(uint32_t crc, const void* msg, int size)
    {
#ifdef ZCRC32_SOFT
      return calc_soft(crc, msg, size);
#endif
#ifdef ZCRC32_LUT
...
#ifdef ZCRC32_LUT
    static uint32_t calc_lut(uint32_t crc, const void* msg, int size)
    {
      for(int i = 0; i < size; i++) {
        crc = crc32_tab[(*((uint8_t*)msg + i) ^ crc) & 0xff] ^ ((crc >> 8));
      }
      return crc;
    }
