...
static void cmd_ls(char* const tokens[])
{
  ...
  if(fs.opendir(&dir, g.pwd) != ZROMFS::OK)
    return;
  if(ll)
    printf("%-8s %-8s %-8s %-8s %-10s %-16s\n", "Offset", "Spec", "Next", "Size",
      "Type", "Filename");
  while(fs.readdir(&dir, &finfo) == ZROMFS::OK) {
    n++;
    tot_size += finfo.fsize;
    if(ll)
      printf("%-8X %-8X %-8X %-8u %-10s %-16s\n", finfo.offset, finfo.spec, finfo.next,
        finfo.fsize, ftype[finfo.ftype], finfo.fname);
    else
...
static void cmd_stat(char* const tokens[])
{
  ...
  if(fs.stat(path, &finfo) != ZROMFS::OK) {
    printf("    File %s not found.\n\n", tokens[1]);
    return;
  }
  printf("%-8s %-8s %-8s %-8s %-10s %-16s\n", "Offset", "Spec", "Next", "Size", "Type",
    "Filename");
  printf("%-8X %-8X %-8X %-8u %-10s %-16s\n\n", finfo.offset, finfo.spec, finfo.next,
    finfo.fsize, ftype[finfo.ftype], finfo.fname);
}
...
{
  ZROMFS::dir_t dir;
  int ret = fs.opendir(&dir, tokens[1]);
  if(ret == ZROMFS::OK) {
  ...
  else if(ret == ZROMFS::DIR_NOT_FOUND)
    printf("%s: No such file or directory.\n\n", tokens[1]);
  else if(ret == ZROMFS::NOT_A_DIR)
